...
You can use this generated SDK by importing from the package `@swastik/dataconnect/react` as shown below. Both CommonJS and ESM imports are supported.
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetUserVariables } from '@swastik/dataconnect';
import { useGetUser } from '@swastik/dataconnect/react'
...
## ListJoinedServers
You can execute the `ListJoinedServers` Query using the following Query hook function, which is defined in [dataconnect-sdk/react/index.d.ts](./index.d.ts):

```javascript
useListJoinedServers(dc: DataConnect, vars: ListJoinedServersVariables, options?: useDataConnectQueryOptions<ListJoinedServersData>): UseDataConnectQueryResult<ListJoinedServersData, ListJoinedServersVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListJoinedServers(vars: ListJoinedServersVariables, options?: useDataConnectQueryOptions<ListJoinedServersData>): UseDataConnectQueryResult<ListJoinedServersData, ListJoinedServersVariables>;
```
...
```
### Return Type
Recall that calling the `ListJoinedServers` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListJoinedServers` Query is of type `ListJoinedServersData`, which is defined in [dataconnect-sdk/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListJoinedServers`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListJoinedServersVariables } from '@swastik/dataconnect';
import { useListJoinedServers } from '@swastik/dataconnect/react'

export default function ListJoinedServersComponent() {
  // The `useListJoinedServers` Query hook requires an argument of type `ListJoinedServersVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListJoinedServers(listJoinedServersVars);
  // Variables can be defined inline as well.
  const query = useListJoinedServers({ userId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListJoinedServers(dataConnect, listJoinedServersVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListJoinedServers(listJoinedServersVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListJoinedServers(dataConnect, listJoinedServersVars, options);
...
You can execute the `ListDiscoverableServers` Query using the following Query hook function, which is defined in [dataconnect-sdk/react/index.d.ts](./index.d.ts):

```javascript
useListDiscoverableServers(dc: DataConnect, vars?: ListDiscoverableServersVariables, options?: useDataConnectQueryOptions<ListDiscoverableServersData>): UseDataConnectQueryResult<ListDiscoverableServersData, ListDiscoverableServersVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListDiscoverableServers(vars?: ListDiscoverableServersVariables, options?: useDataConnectQueryOptions<ListDiscoverableServersData>): UseDataConnectQueryResult<ListDiscoverableServersData, ListDiscoverableServersVariables>;
```
...
```
### Return Type
Recall that calling the `ListDiscoverableServers` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListDiscoverableServers` Query is of type `ListDiscoverableServersData`, which is defined in [dataconnect-sdk/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListDiscoverableServers`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListDiscoverableServersVariables } from '@swastik/dataconnect';
import { useListDiscoverableServers } from '@swastik/dataconnect/react'

export default function ListDiscoverableServersComponent() {
  // The `useListDiscoverableServers` Query hook has an optional argument of type `ListDiscoverableServersVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListDiscoverableServers(listDiscoverableServersVars);
  // Variables can be defined inline as well.
  const query = useListDiscoverableServers({ domain: ..., });
  // Since all variables are optional for this Query, you can omit the `ListDiscoverableServersVariables` argument.
  // (as long as you don't want to provide any `options`!)
  const query = useListDiscoverableServers();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListDiscoverableServers(dataConnect, listDiscoverableServersVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListDiscoverableServers(listDiscoverableServersVars, options);
  // If you'd like to provide options without providing any variables, you must
  // pass `undefined` where you would normally pass the variables.
  const query = useListDiscoverableServers(undefined, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListDiscoverableServers(dataConnect, listDiscoverableServersVars /** or undefined */, options);
  ...
  // If the Query is successful, you can access the data returned using the `UseQueryResult.data` field.
  if (query.isSuccess) {
    console.log(query.data.servers);
  }
  return <div>Query execution {query.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
...
## ListMessages
You can execute the `ListMessages` Query using the following Query hook function, which is defined in [dataconnect-sdk/react/index.d.ts](./index.d.ts):
...
To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListMessages` Query is of type `ListMessagesData`, which is defined in [dataconnect-sdk/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListMessagesVariables } from '@swastik/dataconnect';
import { useListMessages } from '@swastik/dataconnect/react'
...
export default function ListMessagesComponent() {
  ...
  // Variables can be defined inline as well.
  const query = useListMessages({ targetId: ..., });
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, CreateUserVariables } from '@swastik/dataconnect';
import { useCreateUser } from '@swastik/dataconnect/react'
...
export default function CreateUserComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ id: ..., name: ..., email: ..., university: ..., domain: ..., });
...
## JoinServer
You can execute the `JoinServer` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-sdk/react/index.d.ts](./index.d.ts)):
```javascript
useJoinServer(options?: useDataConnectMutationOptions<JoinServerData, FirebaseError, JoinServerVariables>): UseDataConnectMutationResult<JoinServerData, JoinServerVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useJoinServer(dc: DataConnect, options?: useDataConnectMutationOptions<JoinServerData, FirebaseError, JoinServerVariables>): UseDataConnectMutationResult<JoinServerData, JoinServerVariables>;
```
...
```
### Return Type
Recall that calling the `JoinServer` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `JoinServer` Mutation is of type `JoinServerData`, which is defined in [dataconnect-sdk/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `JoinServer`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, JoinServerVariables } from '@swastik/dataconnect';
import { useJoinServer } from '@swastik/dataconnect/react'

export default function JoinServerComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useJoinServer();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useJoinServer(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useJoinServer(options);
  ...
  const mutation = useJoinServer(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useJoinServer` Mutation requires an argument of type `JoinServerVariables`:
  const joinServerVars: JoinServerVariables = {
    userId: ..., 
    serverId: ..., 
  };
  mutation.mutate(joinServerVars);
  // Variables can be defined inline as well.
  mutation.mutate({ userId: ..., serverId: ..., });

  // You can also pass in a `useDataConnectMutationOptions` object to `UseMutationResult.mutate()`.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  mutation.mutate(joinServerVars, options);
...
## SendMessage
You can execute the `SendMessage` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-sdk/react/index.d.ts](./index.d.ts)):
```javascript
...
To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `SendMessage` Mutation is of type `SendMessageData`, which is defined in [dataconnect-sdk/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, SendMessageVariables } from '@swastik/dataconnect';
import { useSendMessage } from '@swastik/dataconnect/react'
...
export default function SendMessageComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ authorId: ..., content: ..., channelId: ..., dmId: ..., });

    ...
    <>
      <AnimatePresence>
        {showStatusModal && (
          <StatusModal 
            ...
            onSave={(text) => {
               console.log("Saving status:", text);
               // Handle status update logically here
            }}
          />
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className="absolute top-2 right-2 flex gap-1">
          <div className="w-6 h-6 rounded-md bg-black/20 flex items-center justify-center backdrop-blur-sm shadow-xl">
             <div className="w-3 h-3 rounded-full bg-brand-indigo animate-pulse" />
          </div>
        </div>
      </motion.div>
    </>
